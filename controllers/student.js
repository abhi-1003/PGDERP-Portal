...
exports.getApplicantsNames = async (req, res) => {
  ...
  const user = await Student.find(
    { applicationFilled: true, course: { $in: courses } },
    {
      name: 1,
      registrationID: 1,
      personalInfoVerified: 1,
      academicsInfoVerified: 1,
      professionalExperienceVerified: 1,
      documentsVerified: 1,
      applicationVerified: 1,
    }
  )
...
exports.getAcademicDetails = async (req, res) => {
  ...
  try {
    if (user && user["academicsInfo"] !== undefined) {
      let acad = Object.assign({}, user["academicsInfo"], {
  ...
  try {
    if (user) {
      if (user["professionalExperience"] !== undefined) {
        let prof = Object.assign(
          {},
          { exps: user["professionalExperience"] },
          { profExp: user.documents.profExp }
        );
        return res.json({
          professionalExperience: prof,
          modifications: user.modifications,
          verified: user.verified,
        });
      }
    ...
    if (user) {
      return res.json({
        otherDocs: {
          ...
          bank: user.feesDetails.bank,
          refNo: user.feesDetails.refNo,
          amt: user.feesDetails.amt,
          date: user.feesDetails.date,
        },
        modifications: user.modifications,
        verified: user.verified,
      });
...
exports.getNoStudentsandEmailCheck = async (req, res) => {
  ...
  Student.findOne({ email }, (err, user) => {
    ...
    Student.countDocuments({ course: course })
...
exports.editStudentInfo = async (req, res) => {
  if (req.userRole == "student") {
    ...
    try {
      ...
      if (req.body["message"] === "Personal Info Completed") {
        return res.send({
          message: "Personal Information Details Saved Successfully",
        });
      }
      if (req.body["message"] === "Academics Info Completed") {
      ...
      if (req.body["message"] === "Professional Experience Completed") {
        return res.send({
          message: "Professional Experience Details Saved Successfully",
        });
      }
      if (req.body["message"] === "Fees Details Completed") {
...
exports.fullComplete = async (req, res) => {
  if (req.userRole == "student") {
    ...
    try {
      const user = await Student.findById(id).exec();
      if (
        user["personalInfoFilled"] &&
        user["academicsInfoFilled"] &&
        user["professionalExperienceFilled"] &&
        user["documentsFilled"] &&
        user["feesDetailsFilled"]
      ) {
...
exports.changeVerificationStatus = async (req, res) => {
  ...
  try {
    if (user) {
      if (
        user.personalInfoVerified &&
        user.academicsInfoVerified &&
        user.professionalExperienceVerified &&
